...
There are two companion apps, both the same idea with no shared code, because
neither platform can shell out to ffmpeg:

- [conv-ios](https://github.com/gueboba/conv-ios) for iPhone and iPad, built on
  Apple's own frameworks. Saves to Photos, to Files, or to a share sheet.
- [conv-android](https://github.com/gueboba/conv-android) for Android, built on
  Android's. Saves to the gallery, to Downloads, or to a share sheet, and
  declares no permissions at all — without the internet permission the system
  will not open a socket for it.

Both convert the same six categories and run entirely offline.
